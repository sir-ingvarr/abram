document.addEventListener('DOMContentLoaded', function() {
    ...
    const planet1 = new Planet({ radius: 25, rotateRadius: 150, speed: 0.0001231, color: new RGBAColor(150, 20, 30) });
    engine.AppendGameObject(planet1);

    const planet2 = new Planet({ radius: 30, rotateRadius: 278, speed: 0.002934, color: new RGBAColor(0, 100, 30) });
    ...
    const planet4 = new Planet({ radius: 10, rotateRadius: 50, speed: 0.0011934, color: new RGBAColor(200, 250, 250) });
    planet3.AppendChild(planet4);

    const lines = new Lines({ planet1, planet2, limit: 5000 });
    engine.AppendGameObject(lines);

    const lines2 = new Lines({ planet1: planet2, planet2: planet3, limit: 5000 });
    engine.AppendGameObject(lines2);

    const lines3 = new Lines({ planet1: planet3, planet2: planet4, limit: 1000 });
